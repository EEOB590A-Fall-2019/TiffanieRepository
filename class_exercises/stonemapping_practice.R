...
#---
# Practice plotting
UScounty <- map('hardin', 'iowa', fill = TRUE, col = palette())
#---
...
ggplot() +
   ...
   # scale lat and long correctly regardless of panel/window size - Essential for scaling!
   coord_quickmap() +
   # remove background colors and grid lines
   theme_classic()


# Change lines and colors on the map - color outlines only the edges of each indiv area
windows()
ggplot() +
   geom_polygon(data = italy,
                aes(x = long, y = lat, group = group),
